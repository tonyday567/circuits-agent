...
-- | Multi-agent communication channel built on 'Circuit.Agent.Process' primitives.
--
-- === Architecture
--
...
import Circuit.Agent.Framing (formatNow, frameMessage, parseMessage)
...
import Circuit.Ends (Ends (..), HasUnit (..), In (..), Out (..), emit, open)
...
-- writes to stdout (which the process config redirects to the log file).
-- We keep a persistent write handle open so @cat@ never sees EOF
-- and stays alive across messages.
toProcessConfig :: ChannelConfig -> ReplConfig
toProcessConfig cfg =
  defaultReplConfig
    { replCommand = "cat",
      replArgs = [],
...
--
-- Wraps a 'ProcessPorts' stdout cursor (for reading) with a persistent
-- write handle to the FIFO.  Keeping the write-end open prevents the bus
-- from exiting on EOF between messages.
data Channel = Channel
  { chProc :: ProcessPorts [Text] [Text] [Text],
    chCfg :: ChannelConfig,
    chWriteH :: Handle
  }
...
channelAttach cfg = do
  pp <- attachProcessPorts (toProcessConfig cfg)
...
channelRecvRaw ch = runKleisli (emit (peOut (chProc ch)) inU) ()
  where
    Ends inU _ = open
